...
async function googleSearch(query) {
  if (!SERP_KEY) return [];
  try {
    const u = `https://serpapi.com/search.json?api_key=${SERP_KEY}&engine=google&q=${encodeURIComponent(query)}&gl=in&hl=en&num=10`;
    const r = await httpsGet(u);
    console.log('SerpApi status:', r.status);
    if (r.status !== 200) { console.log('SerpApi error:', r.body.slice(0,200)); return []; }
    const d = JSON.parse(r.body);
    const results = (d.organic_results || []).map(i => ({ title: i.title || '', snippet: i.snippet || '', url: i.link || '', displayUrl: i.displayed_link || '' }));
    console.log('SerpApi results:', results.length);
    return results;
  } catch (e) { console.log('SerpApi failed:', e.message); return []; }
}
...
http.createServer(async (req, res) => {
  ...
  if (req.url === '/health') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ status: 'ok', groq: !!GROQ_KEY, serp: !!SERP_KEY }));
    return;
  }
